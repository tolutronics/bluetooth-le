...
class BluetoothLe : Plugin() {
    ...
    private var connectionReceiver: BluetoothConnectionReceiver? = null
    ...
    // Bluetooth Connection Monitoring (Background Support)

    private fun registerConnectionReceiver() {
        if (connectionReceiver == null) {
            connectionReceiver = BluetoothConnectionReceiver()
            BluetoothConnectionReceiver.setPluginInstance(this)
            ...
            Logger.debug(TAG, "Bluetooth connection receiver registered")
        }
    }
    ...
    private fun unregisterConnectionReceiver() {
        if (connectionReceiver != null) {
            try {
                ...
                BluetoothConnectionReceiver.setPluginInstance(null)
                connectionReceiver = null
                Logger.debug(TAG, "Bluetooth connection receiver unregistered")
            } catch (e: Exception) {
                Logger.error(TAG, "Error unregistering connection receiver", e)
            }
        }
    }

    // Called by BluetoothConnectionReceiver when device connects/disconnects
    fun notifyBluetoothConnectionChange(connected: Boolean, deviceId: String, deviceName: String?) {
        val data = JSObject()
        ...
        try {
            notifyListeners("bluetoothConnectionChange", data)
            Logger.debug(TAG, "Notified listeners: bluetoothConnectionChange - ${if (connected) "CONNECTED" else "DISCONNECTED"} - $deviceName [$deviceId]")
        } catch (e: ConcurrentModificationException) {
            Logger.error(TAG, "Error in notifyListeners", e)
        }
    }
}
