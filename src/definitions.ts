...
export interface BluetoothConnectionChangeEvent {
  ...
  connected: boolean;
  /**
   * The MAC address of the Bluetooth device (Android)
   */
  deviceId: string;
  /**
  ...
  deviceName?: string;
  /**
   * Timestamp when the connection change occurred
   */
  timestamp: number;
}
...
export interface BluetoothLePlugin {
  ...
  addListener(
    eventName: 'bluetoothConnectionChange',
    listenerFunc: (event: BluetoothConnectionChangeEvent) => void,
  ): Promise<PluginListenerHandle>;
