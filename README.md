...
<p align="center"><strong><code>@tolutronics/bluetooth-le</code></strong></p>
<p align="center">
...
<p align="center">
  ...
  <a href="https://github.com/tolutronics/bluetooth-le/actions?query=workflow%3A%22CI%22"><img src="https://img.shields.io/github/actions/workflow/status/tolutronics/bluetooth-le/main.yml?branch=main&style=flat-square" /></a>
  <a href="https://www.npmjs.com/package/@tolutronics/bluetooth-le"><img src="https://img.shields.io/npm/l/@tolutronics/bluetooth-le?style=flat-square" /></a>
<br>
  <a href="https://www.npmjs.com/package/@tolutronics/bluetooth-le"><img src="https://img.shields.io/npm/dw/@tolutronics/bluetooth-le?style=flat-square" /></a>
  <a href="https://www.npmjs.com/package/@tolutronics/bluetooth-le"><img src="https://img.shields.io/npm/v/@tolutronics/bluetooth-le?style=flat-square" /></a>
...
| ------ | --------- | ------------------------------------------------------------------------- |
| 7.x    | 7.x       | [README](https://github.com/tolutronics/bluetooth-le/blob/main/README.md) |
...
- Provide faster bug fixes and community support
- Customize the plugin for specific use cases and requirements
...
- **Cross-platform support**: Works on Web, Android, and iOS with a unified API
- **Capacitor 7 compatible**: Built for the latest Capacitor framework
- **Comprehensive BLE operations**: Read, write, notify on characteristics and descriptors
- **Advanced scanning**: Filter devices by services, name, manufacturer data
- **Bonding support**: Create and manage device bonds (Android)
- **Connection management**: Configurable timeouts, connection priority, MTU settings
- **Device discovery**: Multiple scan modes, duplicate handling, RSSI reading
- **Android 12+ ready**: Optional location-free scanning with proper permissions
- **Background support**: iOS background mode for continuous BLE operations
...
```
npm install @tolutronics/bluetooth-le
npx cap sync
```
...
- Set the `androidNeverForLocation` flag to `true` when initializing the `BleClient`.
  ```ts
  import { BleClient } from '@tolutronics/bluetooth-le';
...
```typescript
import { BleClient, numbersToDataView, numberToUUID } from '@tolutronics/bluetooth-le';
...
```typescript
import { BleClient, numberToUUID } from '@tolutronics/bluetooth-le';
...
### Scanning with manufacturer data filter (v7.1.0+)

You can now filter devices by manufacturer data for more precise device discovery:

```typescript
import { BleClient } from '@tolutronics/bluetooth-le';

export async function scanWithManufacturerData(): Promise<void> {
  try {
    await BleClient.initialize();
...
### Using configurable timeout for notifications (v7.2.0+)

You can now specify a custom timeout when starting notifications:
...
export async function startNotificationsWithTimeout(deviceId: string): Promise<void> {
  try {
    // Set a custom timeout of 15 seconds for startNotifications
    await BleClient.startNotifications(
      deviceId,
      HEART_RATE_SERVICE,
...
```typescript
import { numberToUUID } from '@tolutronics/bluetooth-le';
...
This plugin is a fork of [@capacitor-community/bluetooth-le](https://github.com/capacitor-community/bluetooth-le). We are grateful to all the original contributors who built and maintained the foundation of this excellent plugin. All original contributors are acknowledged below.
